...
def identity(title: str, item: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    colors = re.findall(r"\b(черн\w*|бел\w*|син\w*|красн\w*|розов\w*|зелен\w*|сер\w*|желт\w*|фиолет\w*|коричнев\w*)\b", text)
...
def match_score(a: dict[str, Any], b: dict[str, Any]) -> float:
    if a.get("barcode") and b.get("barcode") and a["barcode"] == b["barcode"]:
        return 1.0
    if a.get("article") and b.get("article") and a["article"].lower() == b["article"].lower():
        return 0.98
