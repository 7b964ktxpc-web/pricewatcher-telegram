...
def _iter_jsonld(raw_html: str):
    for block in re.findall(r"<script[^>]+type=[\"']application/ld\+json[\"'][^>]*>(.*?)</script>", raw_html, re.I | re.S):
        ...
        elif isinstance(data, dict) and isinstance(data.get("@graph"), list): yield from data["@graph"]
        else: yield data
...
def _product_nodes(raw_html: str) -> list[dict[str, Any]]:
    ...
    for node in _iter_jsonld(raw_html):
        ...
        if any(str(t).casefold() == "product" for t in types): nodes.append(node)
    return nodes
...
def _fallback_prices(raw_html: str) -> list[float]:
    ...
    for match in PRICE_RE.findall(_clean(raw_html)):
        value = _price(match)
        if value is not None and 1 <= value <= 10_000_000: prices.append(value)
...
def extract_product_page(page: dict[str, Any], query: str = "") -> list[dict[str, Any]]:
    ...
    for node in _product_nodes(raw):
        ...
        image = _first(node.get("image")); product_url = node.get("url") or url
        ...
        results.append(normalize_product(source=source, marketplace=source, product_id=node.get("sku") or node.get("mpn") or product_url,
