...
def _offer_price_data(offers: Any) -> tuple[float | None, float | None, str]:
    """Return current price, comparable old/list price and availability from schema.org Offer data."""
    offer = _first(offers)
    if not isinstance(offer, dict):
        return None, None, ""

    price = _price(offer.get("price"))
    old_price = _price(offer.get("highPrice"))
    specification = offer.get("priceSpecification")
    if isinstance(specification, list):
        specification = _first(specification)
    if isinstance(specification, dict):
        old_price = old_price or _price(specification.get("highPrice"))

    availability = str(offer.get("availability") or "").casefold()
    return price, old_price, availability
...
def extract_product_page(page: dict[str, Any], query: str = "") -> list[dict[str, Any]]:
    ...
    for node in _product_nodes(raw):
        ...
        price, old_price, availability = _offer_price_data(offers)
        image = _first(node.get("image"))
        product_url = node.get("url") or url
        title = _clean(node.get("name")) or _fallback_title(raw) or query
        available = None if not availability else not any(
            x in availability for x in ("outofstock", "soldout", "unavailable")
        )
    ...
    if not raw or not url or (not title and not prices):
        return []
