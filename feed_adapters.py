...
class FeedAdapter:
    ...
    def search(self, query: str, limit: int = 20) -> dict[str, Any]:
        ...
        try:
            r = requests.get(url, timeout=float(os.getenv("PARSER_TIMEOUT", "20")), headers={"User-Agent": os.getenv("PARSER_USER_AGENT", "MarketplaceParser/1.0"), "Accept": "application/json, application/xml, text/xml, */*"})
            r.raise_for_status()
    ...
    def _parse(self, text: str, content_type: str) -> list[dict[str, Any]]:
        ...
        if "json" in content_type or stripped.startswith("[") or stripped.startswith("{"):
            ...
            for offer in root.findall(".//offer"):
                raw.append({"id": offer.attrib.get("id"), "title": offer.findtext("name") or offer.findtext("model") or "", "price": offer.findtext("price"), "old_price": offer.findtext("oldprice"), "url": offer.findtext("url"), "available": offer.attrib.get("available", "true") != "false", "category": offer.findtext("categoryId"), "description": offer.findtext("description"), "image": offer.findtext("picture")})
        out = []
        for p in raw if isinstance(raw, list) else []:
            if not isinstance(p, dict):
                continue
            out.append(normalize_product(source=self.name, marketplace=self.marketplace, product_id=p.get("id") or p.get("sku") or p.get("offer_id"), title=p.get("title") or p.get("name"), price=p.get("price"), old_price=p.get("old_price") or p.get("oldprice"), url=p.get("url"), category=p.get("category"), available=p.get("available", True), extra={"description": p.get("description"), "image": p.get("image") or p.get("picture")}))
        return out
