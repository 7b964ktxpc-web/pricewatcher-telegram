...
def test_check_once_updates_price_and_notifies_on_drop(monkeypatch) -> None:
    ...
    class Response:
        ...
        def json(self) -> dict:
            return {"confirmed": [{"price": 4200}, {"price": 4500}]}
    ...
    monkeypatch.setattr(
        watchlist_checker,
        "update_price",
        lambda chat_id, item_key, price: updates.append((chat_id, item_key, price)),
    )
    monkeypatch.setattr(
        watchlist_checker,
        "send_message",
        lambda chat_id, text, **kwargs: sent.append((chat_id, text)),
    )
...
def test_check_once_does_not_notify_when_price_is_unchanged(monkeypatch) -> None:
    ...
    class Response:
        ...
        def json(self) -> dict:
            return {"items": [{"price": 3000}, {"price": 3200}]}
